...
export const advanceStepAllowed = (errors: Record<string, string>) =>
  isAnyTrue(Object.values(errors));
...
export const validateEmail = async ({ email }: Pick<User, "email">) => {
  ...
  return (await emailSchema.isValid({ email }))
    ? ""
...
export const validatePhoneNumber = ({
  ...
  return phoneNumbeValid({ phoneNumber })
    ? ""
...
}: Pick<User, "passwordConfirmation" | "password">) => {
  if (!password) return "Password cannot be empty.";
  if (!passwordConfirmation)
    return "Password confirmation must not be empty and must match password.";
  if (passwordConfirmation !== password)
    return "Password confirmation and password must match, please check and resubmit.";
};
